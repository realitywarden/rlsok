...
export function createFanucFixture(now = new Date()): { profile: Profile; observation: Observation; proposals: ProposalBatch } {
  ...
  return { profile,
    observation: { schemaVersion: 1, profileId: profile.id, observedAt: timestamp, collector: 'fixture/v1', environment: { ...profile.environment },
      ...
      paths: profile.paths.filter(p => 'actionType' in p).map(p => ({ id: p.id, endpoint: p.endpoint, actionType: p.actionType, interfaceSha256: p.interfaceSha256, serverCount: 1 })) },
...
export function createFanucPublicFixture(now = new Date()) {
  ...
  fixture.observation.paths = fixture.profile.paths.filter(p => 'actionType' in p).map(p => ({ id: p.id, endpoint: p.endpoint, actionType: p.actionType, interfaceSha256: p.interfaceSha256, serverCount: 1 }));
  return fixture;
}
