...
export type ProjectFileInspection = {
  ...
  model?: string; movableJoints?: string[]; needsExpansion?: boolean;
...
export function inspectProjectFile(name: string, base64: string): ProjectFileInspection {
  ...
  if (!/\.(?:json|ya?ml)$/i.test(name)) return result;
  result.kind = 'configuration';
  let document: unknown;
  try { const source = new TextDecoder('utf-8', { fatal: true }).decode(bytes); document = /\.json$/i.test(name) ? JSON.parse(source) : yaml.load(source); }
  catch { result.warnings.push('Configuration structure could not be parsed; exact bytes can still be checked.'); return result; }
