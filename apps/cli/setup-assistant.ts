...
const token=${safeToken};let catalog=null,catalogOrigin='none',fragments=[],workspace=null,formControls=null,projectFiles=[],inspections=[],folderCandidates=[],folderCatalogFiles=[],folderTemplateFiles=[],folderXacroFiles=[],folderXacroResources=[];
...
function showProject(){const suggestion=projectSuggestions();render('projectList',inspections.map(item=>({title:item.name+' · '+item.kind,detail:[item.model&&'model '+item.model,item.movableJoints?.length&&item.movableJoints.length+' movable joints',item.controllerCandidates.length&&item.controllerCandidates.length+' controller candidates',...item.warnings].filter(Boolean).join(' · ')||'File recognized; no safe configuration assumption.'})));
  show('projectStatus',inspections.length+' files inspected. '+(suggestion.robot?'Expanded robot description recognized.':suggestion.robotCount>1?'Several expanded robot descriptions found; choose the intended one.':'Expanded URDF still needed.')+' '+(suggestion.orders.length?'Joint-order candidates found; confirm command order.':'Command joint order still needed.'),suggestion.robot?'good':'bad');updateNextStep()}
...
function ruleEditor(card,value,treeForEndpoint,endpoint){
  ...
  const heading=document.createElement('p');heading.textContent='Pick a scalar field from the installed message definition, then state its actual meaning and unit. Add only fields you intend to check.';card.append(heading);
  ...
  function appendRule(rule={}){const row=document.createElement('section');row.className='path';list.append(row);const fields=[];
    fields.push(field(row,'Message field pointer',rule.pointer||''));fields[0].setAttribute('list',datalist.id);
...
function reusableTemplate(){if(!workspace)throw new Error('Generate a setup plan first.');const id=$('templateId').value.trim(),version=$('templateVersion').value.trim(),name=$('templateName').value.trim();
  ...
  for(const path of workspace.template.paths)if(path.adapter==='topic_fields'){const value=formControls?.paths.find(item=>item.pathId===path.id)?.mapping.rulesJson?.value;let rules;try{rules=JSON.parse(value||'')}catch{throw new Error('Add valid custom topic field rules before saving this template.')}if(!Array.isArray(rules)||!rules.length)throw new Error('Add at least one custom topic field rule before saving this template.')}
...
$('projectFolder').onchange=async event=>{try{
  ...
  projectFiles=[];inspections=[];catalog=null;catalogOrigin='none';workspace=null;folderCandidates=[];folderCatalogFiles=[];folderTemplateFiles=[];folderXacroFiles=[];folderXacroResources=[];renderFinish();render('projectList',[]);$('folderChoices').replaceChildren();$('folderSavedChoices').replaceChildren();renderXacroChoices();show('projectStatus','New folder selected. Inspect the actual files.');renderInterfaceChoices();show('catalogStatus','No catalog loaded for this folder. Discover this ROS graph or import one.');$('download').disabled=true;$('templateDownload').disabled=true;$('preview').textContent='Nothing generated yet.';
