...
import { buildLocalDataWorkspaceArchive, buildSetupWorkspace, type WorkspaceFile } from './setup-workspace';
import { composeLocalDataTemplates, evaluateLocalDataWorkspace, inspectLocalData, localDataTemplateSchema, prepareLocalDataWorkspace } from '../../packages/composable-shadow/local-data';
import { inspectProjectFile } from './setup-project';
import { expandTrustedXacro, type XacroInput } from './setup-xacro';
import { launchBrowser } from './browser-launch';

const MAX_BODY = 2 * 1024 * 1024;
function localDataBytes(base64: unknown): Buffer {
  if (typeof base64 !== 'string' || !base64.length || base64.length > 11_184_812 || !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(base64))
    throw new Error('invalid_local_data_base64');
  const bytes = Buffer.from(base64, 'base64');
  if (!bytes.length || bytes.length > 8 * 1024 * 1024) throw new Error('local_data_source_must_be_nonempty_under_8MiB');
  return bytes;
}
const ADAPTERS = [
...
<div class="notice"><strong>Read-only setup:</strong> ROS discovery reads the visible graph and interface definitions; local data inspection reads a selected JSON/YAML file. Neither path publishes a topic, sends an action goal, or grants execution permission. <a href="#localData">Skip to local data without ROS</a>.</div>
...
</aside></div><section id="finish" class="card" hidden><h2>Finish the check workspace</h2><p>Review detected paths, attach actual files and provide a real example message or goal for each path. Confirm units and meaning from your interface documentation.</p><div id="finishFields"></div><button id="complete">Validate and download workspace ZIP</button><div id="completeStatus" class="status"></div></section>
<section class="card" id="localData"><h2>Local JSON / YAML data — no ROS required</h2><p>Choose a real project data file. The assistant reads its structure locally and prepares a versioned, reusable scalar-field check. This checks file content only: it does not discover a live device or infer units, meaning, producer identity, or robot safety.</p>
<div class="two"><label class="field">Data file<input id="localDataFile" type="file" accept=".json,.yaml,.yml"></label><label class="field">Parser<select id="localDataParser"><option value="json-records/v1">JSON records</option><option value="yaml-records/v1">YAML records</option></select></label></div><label class="field">Or choose a JSON/YAML file found in the opened project folder<select id="localDataProjectFile"><option value="">Open a project folder first</option></select></label><button id="localDataUseProjectFile" class="secondary">Use selected project file</button>
<button id="localDataInspect" class="secondary">Inspect file structure</button><div id="localDataStatus" class="status">Next: choose a JSON or YAML file and inspect its structure.</div><div id="localDataFields" class="list"></div>
<h3>Choose checks and confirm meaning</h3><p>Use a detected scalar field or type an exact JSON pointer. Add a meaningful unit (or an explicit label such as “class name”). Numeric limits and allowlists are optional. Existing templates can be imported and combined; overlapping field rules are rejected.</p>
<div class="two"><label class="field">Field pointer<input id="localDataPointer" list="localDataPointers"></label><label class="field">Type<select id="localDataType"><option>string</option><option>number</option><option>integer</option><option>boolean</option></select></label></div><datalist id="localDataPointers"></datalist>
<div class="two"><label class="field">Actual meaning<input id="localDataMeaning"></label><label class="field">Actual unit / label<input id="localDataUnit"></label></div>
<div class="two"><label class="field">Minimum (optional)<input id="localDataMinimum" type="number" step="any"></label><label class="field">Maximum (optional)<input id="localDataMaximum" type="number" step="any"></label></div><label class="field">Allowed values, one per line (optional)<textarea id="localDataAllowed"></textarea></label>
<button id="localDataAdd" class="secondary">Add confirmed field rule</button><div id="localDataRules" class="list"></div>
<label class="button secondary" for="localDataFragments">Import versioned rule fragments</label><input id="localDataFragments" type="file" accept=".json,application/json" multiple><div id="localDataFragmentList" class="list"></div>
<div class="two"><label class="field">Template ID<input id="localDataTemplateId" value="local-data-check"></label><label class="field">Template version<input id="localDataTemplateVersion" value="1.0.0"></label></div><div class="two"><label class="field">Template name<input id="localDataTemplateName" value="Local data check"></label><label class="field">This machine's ID<input id="localDataDeviceId"></label></div>
<label class="field"><input id="localDataConfirm" type="checkbox">I checked each selected field's meaning, unit, limits and source against this machine's documentation.</label>
<button id="localDataSaveTemplate" class="secondary">Save versioned rule template</button><button id="localDataExport">Validate and download runnable workspace</button><div id="localDataExportStatus" class="status">Still needed: inspected file, selected rules, machine ID and semantic confirmation.</div></section></main><script>
...
$('projectFolder').onchange=async event=>{try{
  ...
  localDataChosenFile=null;invalidateLocalDataSelection(true);
  show('localDataStatus',localDataProjectCandidates.length?'Project contains '+localDataProjectCandidates.length+' JSON/YAML candidate(s). Next: choose the intended file above, click Use selected project file, then Inspect file structure.':'No JSON/YAML data candidate found in this folder. Choose a data file manually if applicable.',localDataProjectCandidates.length?'good':'');
...
let localDataInspection=null,localDataRules=[],localDataFragments=[],localDataProjectCandidates=[],localDataChosenFile=null;
const localDataCurrentFile=()=>localDataChosenFile||$('localDataFile').files[0];
function invalidateLocalDataSelection(clearMachine=false){localDataInspection=null;$('localDataFields').replaceChildren();$('localDataPointers').replaceChildren();$('localDataConfirm').checked=false;if(clearMachine)$('localDataDeviceId').value='';renderLocalDataRules()}
function renderLocalDataRules(){
  const root=$('localDataRules');root.replaceChildren();
  localDataRules.forEach((rule,index)=>{const row=document.createElement('div'),label=document.createElement('span'),remove=document.createElement('button');row.className='item';label.textContent=rule.pointer+' · '+rule.type+' · '+rule.meaning+' · '+rule.unit;remove.type='button';remove.className='secondary';remove.textContent='Remove';remove.onclick=()=>{localDataRules.splice(index,1);renderLocalDataRules()};row.append(label,remove);root.append(row)});
  const fragmentsRoot=$('localDataFragmentList');fragmentsRoot.replaceChildren();localDataFragments.forEach((fragment,index)=>{const row=document.createElement('div'),label=document.createElement('span'),remove=document.createElement('button');row.className='item';label.textContent=fragment.metadata.name+' · '+fragment.metadata.id+'@'+fragment.metadata.version+' · '+fragment.check.rules.length+' rules';remove.type='button';remove.className='secondary';remove.textContent='Remove';remove.onclick=()=>{localDataFragments.splice(index,1);renderLocalDataRules()};row.append(label,remove);fragmentsRoot.append(row)});
  show('localDataExportStatus','Recognized: '+(localDataInspection?localDataInspection.fields.length+' scalar fields in '+localDataInspection.records+' record(s)':'no inspected file')+'. Still needed: '+(!localDataRules.length&&!localDataFragments.length?'a confirmed field rule or imported fragment; ':'')+'machine ID and semantic confirmation.');
}
$('localDataFile').onchange=()=>{localDataChosenFile=null;invalidateLocalDataSelection(true);const file=localDataCurrentFile();if(file){$('localDataParser').value=file.name.toLowerCase().endsWith('.json')?'json-records/v1':'yaml-records/v1';show('localDataStatus','Selected '+file.name+'. Next: inspect its structure, enter this machine ID and reconfirm rules, meaning and units.')}else show('localDataStatus','Choose a JSON or YAML file.')};
$('localDataProjectFile').onchange=()=>{localDataChosenFile=null;invalidateLocalDataSelection(true);show('localDataStatus','Project file choice changed. Next: click Use selected project file, then inspect its structure.')};
$('localDataUseProjectFile').onclick=()=>{const index=$('localDataProjectFile').value,file=localDataProjectCandidates[Number(index)];if(index===''||!file){show('localDataStatus','Choose a project data file first.','bad');return}localDataChosenFile=file;invalidateLocalDataSelection(true);$('localDataParser').value=file.name.toLowerCase().endsWith('.json')?'json-records/v1':'yaml-records/v1';show('localDataStatus','Selected '+(file.webkitRelativePath||file.name)+'. Next: inspect its structure, enter this machine ID and reconfirm rules, meaning and units.')};
$('localDataParser').onchange=()=>{invalidateLocalDataSelection();show('localDataStatus','Parser changed. Next: inspect the selected file again and reconfirm rules, meaning and units.')};
$('localDataInspect').onclick=async()=>{try{const file=localDataCurrentFile();if(!file)throw new Error('Choose a data file first.');if(file.size>8*1024*1024)throw new Error('File exceeds 8 MiB.');const bytes=new Uint8Array(await file.arrayBuffer());localDataInspection=await api('inspect-local-data',{parser:$('localDataParser').value,base64:base64(bytes)});render('localDataFields',localDataInspection.fields.map(field=>({title:field.pointer,detail:field.type+' · structure only; meaning and unit unknown'})));const choices=$('localDataPointers');choices.replaceChildren();for(const field of localDataInspection.fields)option(choices,field.pointer,field.pointer+' · '+field.type);show('localDataStatus','Recognized '+localDataInspection.fields.length+' scalar fields in '+localDataInspection.records+' record(s). Missing: confirm meanings, units and rules; then enter a machine ID.','good');renderLocalDataRules()}catch(error){localDataInspection=null;show('localDataStatus',error.message||String(error),'bad')}};
$('localDataPointer').onchange=()=>{const field=localDataInspection?.fields.find(item=>item.pointer===$('localDataPointer').value);if(field)$('localDataType').value=field.type};
$('localDataAdd').onclick=()=>{try{const pointer=$('localDataPointer').value.trim(),type=$('localDataType').value,meaning=$('localDataMeaning').value.trim(),unit=$('localDataUnit').value.trim();if(!localDataInspection)throw new Error('Inspect the selected file first.');if(!localDataInspection.fields.some(field=>field.pointer===pointer&&(field.type===type||field.type==='integer'&&type==='number')))throw new Error('Choose a detected scalar pointer and compatible type.');if(!meaning||!unit)throw new Error('Meaning and unit or label are required.');if(localDataRules.some(rule=>rule.pointer===pointer))throw new Error('This field already has a rule.');const rule={pointer,type,meaning,unit};if($('localDataMinimum').value!=='')rule.minimum=Number($('localDataMinimum').value);if($('localDataMaximum').value!=='')rule.maximum=Number($('localDataMaximum').value);const lines=$('localDataAllowed').value.split(String.fromCharCode(10)).map(value=>value.trim()).filter(Boolean);if(lines.length)rule.allowed=lines.map(value=>type==='boolean'?(value==='true'?true:value==='false'?false:(()=>{throw new Error('Allowed boolean must be true or false.')})()):type==='number'||type==='integer'?Number(value):value);const template={schemaVersion:1,kind:'RlsokLocalDataTemplate',metadata:{id:$('localDataTemplateId').value.trim(),version:$('localDataTemplateVersion').value.trim(),name:$('localDataTemplateName').value.trim()},check:{plugin:'scalar-fields/v1',rules:[rule]}};api('validate-local-data-template',{template}).then(()=>{localDataRules.push(rule);renderLocalDataRules();show('localDataStatus','Field rule added. Confirm other fields or download the workspace.','good')}).catch(error=>show('localDataStatus',error.message||String(error),'bad'))}catch(error){show('localDataStatus',error.message||String(error),'bad')}};
$('localDataFragments').onchange=async event=>{try{const selected=await files(event.target);if(localDataFragments.length+selected.length>16)throw new Error('Maximum 16 fragments.');const added=await Promise.all(selected.map(template=>api('validate-local-data-template',{template})));localDataFragments.push(...added);renderLocalDataRules();show('localDataStatus',added.length+' versioned rule fragment(s) imported. Check field meanings for this machine.','good')}catch(error){show('localDataStatus',error.message||String(error),'bad')}};
async function localDataSelectedTemplates(){const metadata={id:$('localDataTemplateId').value.trim(),version:$('localDataTemplateVersion').value.trim(),name:$('localDataTemplateName').value.trim()},templates=[...localDataFragments];if(localDataRules.length)templates.push(await api('validate-local-data-template',{template:{schemaVersion:1,kind:'RlsokLocalDataTemplate',metadata,check:{plugin:'scalar-fields/v1',rules:localDataRules}}}));if(!templates.length)throw new Error('Choose at least one field rule or fragment.');return {metadata,templates}}
$('localDataSaveTemplate').onclick=async()=>{show('localDataExportStatus','Validating versioned rule fragments…');try{const selected=await localDataSelectedTemplates(),template=await api('compose-local-data-templates',selected),blob=new Blob([JSON.stringify(template,null,2)+'\\n'],{type:'application/json'}),anchor=document.createElement('a');anchor.href=URL.createObjectURL(blob);anchor.download='rlsok-local-data-template.json';anchor.click();setTimeout(()=>URL.revokeObjectURL(anchor.href),1000);show('localDataExportStatus','Versioned rule template saved. It contains no source file or machine ID; reimport it for another machine and reconfirm semantics.','good')}catch(error){show('localDataExportStatus',error.message||String(error),'bad')}};
$('localDataExport').onclick=async()=>{show('localDataExportStatus','Validating the selected source and rules…');try{const file=localDataCurrentFile();if(!file||!localDataInspection)throw new Error('Inspect the selected data file first.');if(!localDataRules.length&&!localDataFragments.length)throw new Error('Choose at least one field rule or fragment.');const template={schemaVersion:1,kind:'RlsokLocalDataTemplate',metadata:{id:$('localDataTemplateId').value.trim(),version:$('localDataTemplateVersion').value.trim(),name:$('localDataTemplateName').value.trim()},check:{plugin:'scalar-fields/v1',rules:localDataRules}};const templates=[...localDataFragments];if(localDataRules.length)templates.push(await api('validate-local-data-template',{template}));const bytes=new Uint8Array(await file.arrayBuffer()),payload={parser:$('localDataParser').value,fileName:file.name,base64:base64(bytes),deviceId:$('localDataDeviceId').value.trim(),semanticsConfirmed:$('localDataConfirm').checked,templates,metadata:template.metadata};const response=await fetch('/'+token+'/api/local-data-workspace',{method:'POST',headers:{'content-type':'application/json','x-rlsok-session':token},body:JSON.stringify(payload)});if(!response.ok){const result=await response.json();throw new Error(result.error||'Workspace export failed')}const url=URL.createObjectURL(await response.blob()),anchor=document.createElement('a');anchor.href=url;anchor.download='rlsok-local-data-workspace.zip';anchor.click();setTimeout(()=>URL.revokeObjectURL(url),1000);show('localDataExportStatus','Runnable local workspace downloaded. The report checks this file only; use the included command for a fresh snapshot.','good')}catch(error){show('localDataExportStatus',error.message||String(error),'bad')}};
</script></body></html>`;
}
...
export async function runSetupAssistant(args: string[]): Promise<number> {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (url.pathname === `${base}/api/inspect-local-data` && request.method === 'POST') {
        const input = await body(request, 12 * 1024 * 1024) as { parser?: string; base64?: string };
        json(response, 200, inspectLocalData(input.parser as 'json-records/v1' | 'yaml-records/v1', localDataBytes(input.base64))); return;
      }
      if (url.pathname === `${base}/api/local-data-workspace` && request.method === 'POST') {
        const input = await body(request, 12 * 1024 * 1024) as { parser?: string; fileName?: string; base64?: string;
          deviceId?: string; semanticsConfirmed?: boolean; templates?: unknown[];
          metadata?: { id: string; version: string; name: string } };
        const bytes = localDataBytes(input.base64);
        const workspace = prepareLocalDataWorkspace({ templates: input.templates ?? [],
          parser: input.parser as 'json-records/v1' | 'yaml-records/v1', fileName: input.fileName ?? '', bytes,
          deviceId: input.deviceId ?? '', semanticsConfirmed: input.semanticsConfirmed === true, metadata: input.metadata });
        const report = evaluateLocalDataWorkspace(workspace, bytes);
        if (report.decision !== 'LOCAL_DATA_MATCH') throw new Error(`local_data_initial_check_blocked:${report.violations[0]?.reason ?? 'unknown'}`);
        const archive = buildLocalDataWorkspaceArchive(workspace, input.base64!);
        response.writeHead(200, { 'content-type': 'application/zip', 'content-disposition': 'attachment; filename="rlsok-local-data-workspace.zip"',
          'content-length': archive.length, 'cache-control': 'no-store', 'x-content-type-options': 'nosniff' });
        response.end(archive); return;
      }
      if (url.pathname === `${base}/api/validate-local-data-template` && request.method === 'POST') {
        const input = await body(request) as { template?: unknown };
        json(response, 200, localDataTemplateSchema.parse(input.template)); return;
      }
      if (url.pathname === `${base}/api/compose-local-data-templates` && request.method === 'POST') {
        const input = await body(request) as { templates?: unknown[]; metadata?: { id: string; version: string; name: string } };
        const composed = composeLocalDataTemplates(input.templates ?? []);
        json(response, 200, localDataTemplateSchema.parse({ ...composed, metadata: input.metadata ?? composed.metadata })); return;
      }
