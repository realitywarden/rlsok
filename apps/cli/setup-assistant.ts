...
$('projectFolder').onchange=async event=>{try{
  ...
  const allCandidates=entries.filter(file=>!ignored.test(file.webkitRelativePath||file.name)&&!/^\./.test(file.name)&&/\\.(?:urdf|json|ya?ml)$/i.test(file.name)&&/^[A-Za-z0-9_. -]+$/.test(file.name)&&file.size<=8*1024*1024&&!catalogFiles.includes(file)&&!templateFiles.includes(file));
