...
import { evaluateLocalDataWorkspace, inspectLocalData, localDataTemplateSchema, prepareLocalDataWorkspace } from '../../packages/composable-shadow/local-data';
...
<button id="localDataExport">Validate and download runnable workspace</button><div id="localDataExportStatus" class="status">Still needed: inspected file, selected rules, machine ID and semantic confirmation.</div></section></main><script>
...
function renderLocalDataRules(){
  ...
  render('localDataFragmentList',localDataFragments.map(fragment=>({title:fragment.metadata.name,detail:fragment.metadata.id+'@'+fragment.metadata.version+' · '+fragment.check.rules.length+' rules'})));
