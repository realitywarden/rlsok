...
<div class="card"><h2>4. Review the plan</h2><p>The plan shows recognized interfaces and gaps. The workspace ZIP below includes validated configuration and actual files after you confirm the remaining inputs.</p><button id="download" disabled>Download plan JSON</button><button id="templateDownload" disabled class="secondary">Save reusable template</button><div id="summary" class="status">Next: read project files and discover interfaces.</div><pre id="preview">Nothing generated yet.</pre></div>
...
function projectSuggestions(){
  ...
  return {robot,model:robot?.model||'',controllers:[...new Set(configs.flatMap(item=>item.controllerCandidates))],orders:[...configs.flatMap(item=>item.jointOrderCandidates),...(robot?.movableJoints?.length?[robot.movableJoints]:[])]};
}
...
function renderFinish(){
  ...
  if(!workspace){$('finish').hidden=true;return}
  $('finish').hidden=false;
...
async function finish(){
  ...
  const response=await fetch('/'+token+'/api/workspace',{method:'POST',headers:{'content-type':'application/json','x-rlsok-session':token},body:JSON.stringify({connection,template:workspace.template,files:chosen.map(file=>({path:file.path,base64:base64(file.bytes)}))})});
...
$('templateDownload').onclick=()=>{if(!workspace)return;const blob=new Blob([JSON.stringify(workspace.template,null,2)+'\\n'],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='rlsok-connection-template.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)};
