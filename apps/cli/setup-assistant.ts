...
$('projectFolder').onchange=async event=>{try{
  ...
  localDataChosenFile=null;localDataInspection=null;$('localDataFields').replaceChildren();$('localDataPointers').replaceChildren();renderLocalDataRules();
...
$('localDataFile').onchange=()=>{localDataChosenFile=null;localDataInspection=null;const file=localDataCurrentFile();if(file){$('localDataParser').value=file.name.toLowerCase().endsWith('.json')?'json-records/v1':'yaml-records/v1';show('localDataStatus','Selected '+file.name+'. Next: inspect the file structure.')}else show('localDataStatus','Choose a JSON or YAML file.');$('localDataFields').replaceChildren();$('localDataPointers').replaceChildren();renderLocalDataRules()};
$('localDataUseProjectFile').onclick=()=>{const index=$('localDataProjectFile').value,file=localDataProjectCandidates[Number(index)];if(index===''||!file){show('localDataStatus','Choose a project data file first.','bad');return}localDataChosenFile=file;localDataInspection=null;$('localDataParser').value=file.name.toLowerCase().endsWith('.json')?'json-records/v1':'yaml-records/v1';$('localDataFields').replaceChildren();$('localDataPointers').replaceChildren();show('localDataStatus','Selected '+(file.webkitRelativePath||file.name)+'. Next: inspect its structure.');renderLocalDataRules()};
