...
| E24 GOLEM/H12 | `1a0be8147c5d63cb` | CorrellLab PR #1 and immutable release `h12-lowstate-observer-v1` subscribe-only H1-2 `rt/lowstate` snapshot, plus the existing `capabilities.ts` external-verdict normalizer | Project-specific observer released and delivered; awaiting an owner-run `h12-lowstate-status.json` from the real H1-2. Contact/readiness are not inferred. |
...
| GOLEM/H12 / `1a01d8e3a7eb0fef` | CorrellLab offered GOLEM as a test bed and explicitly allowed a branch/fork test. CorrellLab PR #1 and immutable release `h12-lowstate-observer-v1` contain the subscribe-only H1-2 observer; correction email `1a0be8147c5d63cb` gives the direct download, checksum, guide and one exact run command. | One owner-run `h12-lowstate-status.json` from the normally connected real H1-2. That result can establish real stream observation only; an adapter-owned contact/readiness verdict is still required before such a capability can be claimed. |
...
Release `h12-lowstate-observer-v1` contains the 5,492-byte ZIP with SHA-256
`c7ab62203999f2089ee42f9a68e2a8b227001c8b62abeb9128d529a84e43909c`.
The exact extracted archive passed all three focused tests and Python
compilation.  Website commit `55b7828c1e4e3093396f9125031dac595089734b`
was deployed to production as `dpl_B63ZDWZeSkREzmhT47NfH4aFozoQ`.  The
production guide, Learn index and direct release asset returned HTTP 200; the
asset returned 5,492 bytes and the guide exposed the exact checksum, topic and
`not_inferred` boundary.  Local web typecheck/build and the Vercel production
build completed successfully.
