...
| E24 | Max Conway / CorrellLab GOLEM — 2026-08-21 — Gmail `1a0262287140648b`; meeting request 2026-08-27 `1a0406eea949903b`; material request `1a0bd8575e2d03f2`; PR delivery `1a0be79d13813d43`; v1 delivery `1a0be8147c5d63cb`; v2 source-binding correction `1a0be87d9fbc97ea` | Upper-body motor/capability state, including contact/caught conditions, should be checked before a new H12 motion. The public CorrellLab H1-2 controller was found independently and confirms `rt/lowstate` plus upper-body motor indices 12–26. | A subscribe-only owner-run observation can prove real state-stream availability and record bounded upper-body telemetry. Core still must not infer contact/caught or next-move readiness from raw motors. | CorrellLab PR #1 and immutable release `h12-lowstate-observer-v2` provide the project-specific bounded subscriber at exact upstream commit `6fef8760741af122c59ca13e5f1960a1f79b230a`; v2 also requires the checkout root actually in use and records its commit/dirty state/origin. It creates no publisher, command message, motion-switcher client or robot action call. Focused offline and extracted-archive tests pass; the corrected website guide is deployed. **PROJECT-SPECIFIC OBSERVER RELEASED; OWNER RUN OPEN**; contact/readiness remain explicitly `not_inferred`. | Max received the v2 download, checksum and corrected command before reporting a run. Await `h12-lowstate-status.json` from a normally connected H1-2; if the controller later owns a grasp/contact verdict, map that field rather than guessing from torque/current. No video meeting or repeat material request is needed. High external Shadow value. |
...
| E24 GOLEM/H12 | `1a0be87d9fbc97ea` | CorrellLab PR #1 and immutable release `h12-lowstate-observer-v2` subscribe-only H1-2 `rt/lowstate` snapshot with required actual-checkout binding, plus the existing `capabilities.ts` external-verdict normalizer | Corrected project-specific observer released and delivered; awaiting an owner-run `h12-lowstate-status.json` from the real H1-2. Contact/readiness are not inferred. |
...
| GOLEM/H12 / `1a01d8e3a7eb0fef` | CorrellLab offered GOLEM as a test bed and explicitly allowed a branch/fork test. CorrellLab PR #1 and immutable release `h12-lowstate-observer-v2` contain the subscribe-only H1-2 observer; correction email `1a0be87d9fbc97ea` gives the v2 download, checksum and command with the actual H12 checkout root. | One owner-run `h12-lowstate-status.json` from the normally connected real H1-2. That result can establish real stream observation only; an adapter-owned contact/readiness verdict is still required before such a capability can be claimed. |
...
confirmed `max.conway@colorado.edu` address in the original thread.  Because
that command would not be directly runnable from upstream main until merge,
message `1a0be8147c5d63cb` immediately supplied the immutable ZIP, checksum,
production guide and the corrected post-unzip command rather than leaving Max
to discover the missing checkout step.

The v1 observer's robot-facing boundary was correct, but its standalone command
did not require the actual H12 checkout path and therefore could not reliably
record the checkout in use.  It was not silently replaced.  PR update
`7c2003f1d833c11946b9db9801c6dc5b07451f95` and immutable release
`h12-lowstate-observer-v2` require `--source-root` to be the exact Git checkout
root and record its full commit, dirty state, checkout name and origin.  The
6,081-byte v2 ZIP has SHA-256
`9ef44d6888156346e87c9abd8fc4fd344ef4ea9b0f25d8f6182ea38c232c254b`.
The exact extracted archive passed all four focused tests and Python
...
direct v2 release asset returned HTTP 200; the asset returned 6,081 bytes and
the guide exposed the exact checksum and required `--source-root`.  Local web
typecheck/build and the Vercel production build completed successfully.

These commits, release, deployment and SENT IDs prove implementation,
