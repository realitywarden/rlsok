# Compare your own ROS setup

New to RLSOK? [Run the included example first](local-check-start.md). It needs no robot or ROS installation.


Local Check version: **v1.5.0**. Start with one command boundary in an isolated simulation. This guide covers the supplied local evaluator; Cloud-managed production authorization is a separate product path.

For saved configuration and camera/CAN/serial identity review without starting ROS or opening hardware, use [saved setup review](saved-setup-review.md). It includes Piper, MakerMods Metal, SO-101 hardware, Dons Beast and Cartesian single/dual input preparation.
