...
(async () => {
  try {
    ...
    fs.writeFileSync(path.join(stage, 'START-HERE.md'), '# Start here\n\nFor live ROS interface discovery, source your ROS 2 environment and run `bin/rlsok setup-assistant`. Open the project folder in the local browser. The assistant shows recognized robot/interface/configuration inputs, missing facts and the next step; it sends no robot command. See [interface onboarding](docs/interface-onboarding.md) for limits and review steps.\n\nFor JSON/YAML local-file checks without ROS, use the same assistant and see [local data self-service](docs/local-data-self-service.md).\n\nNo robot handy? The [included example guide](docs/local-check-start.md) is still available and needs no account.\n');
    fs.writeFileSync(path.join(stage, 'README.md'), '# RLSOK Local Check\n\nFor your project, run `bin/rlsok setup-assistant`; source a ROS 2 terminal only for live ROS discovery and open its local browser page. It discovers interfaces, reads selected robot/configuration files, and prepares a reviewable check workspace. No account, AI, cloud upload or robot command is required. Start with [local data self-service](docs/local-data-self-service.md) or [interface onboarding](docs/interface-onboarding.md).\n\nThe [included example](docs/local-check-start.md) remains available without a robot or account. This tool is separate from the existing robot runtime and Windows management app. See BUILD-MANIFEST.json for the exact checks performed and their limits.\n');
