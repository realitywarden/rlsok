...
VERSION = '1.5.8'
SOURCE = '34a37de2272959b067e0d04135bc1ef167e007af'
LINUX_SHA = 'f40f21c8d19e59b6e388906577added6e6a7ab5eb24c940d2e04a5df988d87a8'
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='rlsok-windows-local-check-') as temporary:
            ...
            for member in archive:
                ...
                if member.isdir():
                    ...
                    with archive.extractfile(member) as src, target.open('wb') as dst:
                        shutil.copyfileobj(src, dst)
        stage = base / prefix
        if (stage/'VERSION').read_text().strip() != VERSION or (stage/'SOURCE_COMMIT').read_text().strip() != SOURCE:
        ...
        asset = {'name': destination.name, 'sizeBytes': destination.stat().st_size, 'sha256': sha(destination),
                 'version': VERSION, 'sourceCommit': SOURCE, 'packagingSourceCommit': packaging_source, 'published': False}
