...
in Local Check 1.5.7. It compares only selected reported fields and makes no
robot connection. For an already-running MowgliNext graph, the opt-in
[Mowgli status observer](docs/mowgli-readonly-status.md) records the reported
firmware and compares explicitly selected saved settings without starting the
bridge or sending robot commands.
