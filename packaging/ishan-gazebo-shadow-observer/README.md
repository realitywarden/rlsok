# RLSOK warehouse Gazebo Shadow observer v5
...
commit, dirty state and origin itself. The version command must print `5`.
The reader accepts either the verified `/ros_gz_bridge` graph identity or the
standard unknown namespace/name metadata that Fast DDS can expose for a valid
bridge endpoint. In the latter case it proves that the unique, correctly typed
`/cmd_vel` subscriber and `/odom` publisher share the same DDS participant GID
prefix, preserves the unknown identity, and never invents a bridge name. It
then subscribes once to `/odom`. It
