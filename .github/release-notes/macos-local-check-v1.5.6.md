# RLSOK Local Check 1.5.6 for macOS

This is the self-contained Local Check file-review tool for macOS 14 or later.
It includes the exact RLSOK Local Check 1.5.6 source payload and a native Node.js
runtime for each processor family.

Version 1.5.6 carries the reusable fixes learned from recent feedback into the
public installer: source-checkout provenance, additional read-only observers,
safer Fast DDS participant matching when node names are anonymous, and the
corrected Workbench offline-review command boundary. Profiles remain opt-in and
fail closed when the selected source or observed identity is ambiguous.

The native SwiftUI local console keeps the current dark, high-density workspace
hierarchy, shows the exact package/source identity and zero-dispatch boundary,
runs the included example, keeps a visible session audit, and opens the local
reports and guide. It does not invent Cloud management or robot-connection
capabilities that are absent from this package.

Choose `arm64` for Apple silicon or `x64` for an Intel Mac. Install the matching
`.pkg`, open **RLSOK Local Check 1.5.6** in Applications, and choose **Run
