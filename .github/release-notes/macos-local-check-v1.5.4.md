# RLSOK Local Check 1.5.4 for macOS

This is the self-contained Local Check file-review tool for macOS 14 or later. It includes the exact RLSOK Local Check 1.5.4 source payload and a native Node.js runtime for each processor family.

Choose `arm64` for Apple silicon or `x64` for an Intel Mac. Install the matching `.pkg`, open **RLSOK Local Check 1.5.4** in Applications, and choose **Run example**. The included example needs no robot, RLSOK account, Node installation or ROS environment. It writes a matching report and a changed-calibration report locally and sends no robot command.

Version 1.5.4 adds the bounded-operation saved-input review for declaring local adjustments, terminating conditions and optional temporal-state reference inputs while keeping live control and independent safety outside RLSOK.
